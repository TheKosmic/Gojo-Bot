...
const smsRowner = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO YO COMO CREADOR(A) DE BOT LO PUEDE USAR!!\`\`\`` }//NUMERO DE BOT
const smsOwner = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO MI CREADOR(A) LO PUEDE USAR!!\`\`\`` }//OWNER
const smsMods = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO MODERADORES Y MI CREADOR(A) LO PUEDEN USAR!!\`\`\`\`` }//MODERADORES
const smsPremium = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO ESTA DISPONIBLE PARA USUARIOS PREMIUM Y MI CREADOR(A)!! PARA SER PREMIUM COMPRE UN PASE USANDO #pass premium\`\`\`` }//USUARIOS PREMIUM
const smsGroup = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO SE PUEDE UTILIZAR EN GRUPOS!!\`\`\`` }//PARA GRUPOS
const smsPrivate = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO SE PUEDE UTILIZAR AL PRIVADO!!\`\`\`` }//AL PRIVADO
const smsAdmin = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO ES PARA ADMINS!!\`\`\`` }//ADMINS
const smsBotAdmin = () => { return `\`\`\`¡¡NECESITO SER ADMIN PARA QUE PUEDAS USAR ESTE COMANDO!!\`\`\`` }//BOT CON ADMIN
const smsUnreg = () => { return `\`\`\`¡¡NECESITAS ESTAR REGISTRADO(A) PARA USAR ESTE COMANDO, ESCRIBE #verificar PARA REGISTRARTE!!\`\`\`` }//VERIFICAR
const smsRestrict = () => { return `\`\`\`¡¡ESTE COMANDO ESTÁ RESTRINGIDO POR MI CREADORA(A)!!\`\`\`` }//RESTRINGIR COMANDO

//MENU LISTA
const smsTime = () => { return `Tiempo Actual`}
const smsUptime = () => { return `Funcionando Durante`}
const smsVersion = () => { return `Versión de ${global.gt}`}
...
const smsLista7 = () => { return `🐈 INSTALAR GOJOBOT 🐈`}
...
const smsSrevoke = () => { return '*No se desanimen, ustedes pueden terminar!!*'}
...
//_allantilink.js
const smsTextoYT = () => { return '*ᘜ𝖔𝖏𝖔-𝖇𝖔𝖙* - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 '} 
...
const eGataCoins = () => { return '🐱 GojoCoins' }
const eGataTickers = () => { return '🎫 Gojo Tickets' } 
...
const smsModP2 = () => { return `Habilitar función para que todos puedan usar GojoBot`}
const smsModAd1 = () => { return `🛂 MODO ADMIN`}
const smsModAd2 = () => { return `Solo los Admins podrán usar GojoBot en Grupos`}
...
const smsAfkM4 = () => { return `*NO ETIQUETES A ESTE(A) USUARIO(A)!! ESTÁ INACTIVO(A)*\n`} 
...
const smsAfkTime = () => { return [['día(s)'], ['hora(s)'], ['minuto(s)'], ['segundo(s)']] }
const smsAfkResultTime = smsAfkTime()

//afk-afk.js
const smsAfkQ1 = (usedPrefix, command) => { return `${lenguajeGB['smsAvisoMG']()}*ESCRIBA EL MOTIVO POR EL CUÁL ESTARÁ INACTIVO (AFK)*\n\n*EJEMPLO:*\n*${usedPrefix + command}* Voy a realizar tarea`} 
const smsAfkQ2 = () => { return `${lenguajeGB['smsAvisoMG']()}*MÍNIMO 10 CARACTERES DEBE DE SER EL MOTIVO*`} 
...
const smsJBConexionClose2 = () => { return `${lenguajeGB['smsAvisoFG']()}🔴 *LA CONEXIÓN SE HA CERRADO, TENDRAS QUE VOLVER A CONECTARSE USANDO:*\n#deletesesion (Para borrar los datos y poder volver a solita el QR o el code)`}
...
const smsContacto4 = () => { return 'Contacto Oficial de GojoBot 🐈'}
